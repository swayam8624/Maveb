...
int run(int argc, char** argv) {
    ...
    auto report = aether::mesh::GltfExporter::writeGlb(*asset, validationPath);
    if (!report)
    ...
    if (options->json) {
        std::cout << "{\"ok\":true,\"dryRun\":" << (options->dryRun ? "true" : "false")
                  ...
                  << aether::package::Sha256::hex(*hash) << "\",\"bytes\":" << report->outputBytes
                  << ",\"primitives\":" << report->primitives
                  << ",\"instances\":" << report->instances << ",\"vertices\":" << report->vertices
                  << ",\"triangles\":" << report->triangles
                  << ",\"materials\":" << report->materials << ",\"textures\":" << report->textures
                  << ",\"images\":" << report->images << "}\n";
    } else {
        std::cout << (options->dryRun ? "Validated" : "Exported") << " native GLB with "
                  << report->vertices << " vertices and " << report->triangles << " triangles\n";
    }
    return 0;
}
