...
effectiveGaussianRendererAlpha(double peakOpacity, double squaredMahalanobis) {
    if (!std::isfinite(peakOpacity) || peakOpacity < 0.0 || peakOpacity > 1.0)
        return fail(ErrorCode::invalidArgument,
                    "Gaussian renderer peak opacity must lie in [0,1]");
    ...
    const double alpha =
        std::min(0.99, peakOpacity * std::exp(-0.5 * squaredMahalanobis));
    if (alpha < 1.0 / 255.0)
        return 0.0;
    return alpha;
}
...
gaussianRendererColorUpperBound(const gaussian::Gaussian& primitive) {
    ...
    constexpr std::array<double, 5> degree2PolynomialBound{1.0, 1.0, 2.0, 1.0,
                                                           1.0};
    constexpr std::array<double, 7> degree3PolynomialBound{4.0, 1.0, 4.0, 3.0,
                                                           4.0, 1.0, 3.0};
    ...
    for (std::size_t channel = 0; channel < result.size(); ++channel) {
        ...
        if (primitive.restCount >= 9) {
            for (std::size_t index = 0; index < 3; ++index) {
                const double coefficient =
                    primitive.rest[channel * 15 + index];
        ...
        if (primitive.restCount >= 24) {
            for (std::size_t index = 0; index < 5; ++index) {
                const double coefficient =
                    primitive.rest[channel * 15 + 3 + index];
                magnitude += c2[index] * degree2PolynomialBound[index] *
                             std::abs(coefficient);
            }
        }
        ...
        if (primitive.restCount >= 45) {
            for (std::size_t index = 0; index < 7; ++index) {
                const double coefficient =
                    primitive.rest[channel * 15 + 8 + index];
                magnitude += c3[index] * degree3PolynomialBound[index] *
                             std::abs(coefficient);
            }
        }
...
certifyGaussianPixelRevision(std::span<const double> beforeEffectiveAlphas,
    ...
    if (!std::isfinite(rgbBound))
        return fail(ErrorCode::resourceExhausted,
                    "Gaussian render certificate RGB bound overflow");
