...
std::size_t GaussianSpatialIndex::RegionKeyHash::operator()(
    const world::RegionKey& key) const noexcept {
...
Result<world::RegionKey> GaussianSpatialIndex::regionKey(simd_float3 position) const {
    if (!std::isfinite(position.x) || !std::isfinite(position.y) ||
        ...
        return fail(ErrorCode::corruptData,
                    "Gaussian spatial index received non-finite position");
    }
...
Result<GaussianSpatialIndex>
GaussianSpatialIndex::build(const gaussian::GaussianAsset& asset, float cellSizeMeters) {
...
Result<void> GaussianSpatialIndex::relocateGaussian(std::size_t gaussianIndex,
    ...
    auto found =
        std::find(oldBucket->second.begin(), oldBucket->second.end(), gaussianIndex);
...
GaussianSpatialIndex::indices(world::RegionKey key) const noexcept {
