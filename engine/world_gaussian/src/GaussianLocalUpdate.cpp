...
Result<GaussianLocalUpdateSelection> selectGaussiansForLocalUpdateIndexed(
    ...
    if (spatialIndex.primitiveCount() != asset.gaussians.size()) {
        return fail(ErrorCode::invalidArgument,
                    "Gaussian spatial index primitive count does not match current asset");
    }
    if (spatialIndex.cellSizeMeters() != worldUpdate.cellSizeMeters) {
        return fail(ErrorCode::invalidArgument,
                    "Gaussian spatial index cell size does not match selective-update plan");
    }
    ...
    for (const auto& [key, region] : dirty) {
        for (const std::size_t index : spatialIndex.indices(key)) {
            if (index >= asset.gaussians.size())
                return fail(ErrorCode::corruptData,
                            "Gaussian spatial index contains out-of-range primitive index");
            ++result.inspectedGaussians;
