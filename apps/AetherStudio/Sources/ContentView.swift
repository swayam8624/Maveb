  ...
  var body: some View {
    StudioPanel {
      VStack(alignment: .leading, spacing: 12) {
        ...
        vectorRow(
          "Position", [binding(\.translationX), binding(\.translationY), binding(\.translationZ)])
        vectorRow(
          "Quaternion",
          [binding(\.rotationX), binding(\.rotationY), binding(\.rotationZ), binding(\.rotationW)])
  ...
  var body: some View {
    StudioPanel {
      VStack(alignment: .leading, spacing: 12) {
        ...
        factorRow(
          "Base RGBA",
          [binding(\.baseRed), binding(\.baseGreen), binding(\.baseBlue), binding(\.baseAlpha)])
        factorRow(
          "Emissive", [binding(\.emissiveRed), binding(\.emissiveGreen), binding(\.emissiveBlue)])
  ...
  var body: some View {
    StudioPanel {
      VStack(alignment: .leading, spacing: 12) {
        ...
        vectorRow(
          "Direction", [binding(\.directionX), binding(\.directionY), binding(\.directionZ)])
...
struct ContentView: View {
  ...
  private var workspaceHeader: some View {
    HStack(spacing: 12) {
      ...
      if let scenePath = document.state.scenePath {
        StatusPill(
          text: URL(fileURLWithPath: scenePath).lastPathComponent,
  ...
  private var emptyViewportState: some View {
    VStack(spacing: 14) {
      ...
      VStack(spacing: 5) {
        ...
        Text(
          "Import an AETHER capture, Gaussian PLY, or glTF asset to activate the Metal viewport."
        )
    ...
    .overlay {
      RoundedRectangle(cornerRadius: 20, style: .continuous).stroke(Color.primary.opacity(0.08))
    }
  }
    ...
    StudioPanel {
      VStack(alignment: .leading, spacing: 5) {
        ...
        if let selectedGaussianId {
          Text("Gaussian #\(selectedGaussianId)").font(.caption2.monospacedDigit()).foregroundStyle(
            .secondary)
        }
        if let selectedMeshId {
          Text("Mesh entity #\(selectedMeshId)").font(.caption2.monospacedDigit()).foregroundStyle(
            .secondary)
        }
      }
    }
        ...
        StudioPanel {
          HStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 8) {
              ...
              Text(
                "AETHER and Gaussian PLY files use the Gaussian path. glTF and GLB files use the mesh/PBR renderer."
              )
              .font(.callout).foregroundStyle(.secondary)
              HStack(spacing: 6) {
                fileTypeChip(".aether")
              ...
              Button {
                ...
                Label("Choose Scene…", systemImage: "folder")
              }
              ...
              if isCapturedScene {
                ...
                HStack {
                  ...
                  Button {
                    ...
                    Label(
                      document.state.dynamicMeshPath == nil ? "Attach Mesh…" : "Replace…",
      ...
      Menu {
        if isCapturedScene {
          Button {
            ...
            Label(
              document.state.dynamicMeshPath == nil ? "Add Dynamic Mesh…" : "Replace Dynamic Mesh…",
              systemImage: "cube.badge.plus")
          }
        ...
        if shadowDebugMode != .disabled {
          Stepper(
            "Shadow Slice \(shadowDebugSlice)", value: $shadowDebugSlice,
  ...
  private func restoreProjectUIState() {
    ...
    gaussianDebugMode =
      GaussianDebugMode(rawValue: document.state.viewport.gaussianDebugMode) ?? .appearance
    shadowDebugMode =
      ShadowDebugMode(rawValue: document.state.viewport.shadowDebugMode) ?? .disabled
  ...
  private var viewportModeLabel: String {
    ...
    case "ply", "aether":
      return document.state.dynamicMeshPath == nil
        ? "STANDARD GAUSSIAN GPU" : "HYBRID GAUSSIAN / DYNAMIC PBR"
  ...
  private func removeSelectedLight() {
    guard document.state.lights.count > 1,
      document.state.lights.indices.contains(selectedLightId - 1)
    else { return }
