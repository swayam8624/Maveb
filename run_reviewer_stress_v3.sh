...
RESIDUAL_BUDGET_FRACTION="${MAVEB_REVIEWER_V3_RESIDUAL_BUDGET_FRACTION:-0.5}"
REUSE="${MAVEB_REVIEWER_V3_REUSE:-1}"
...
for required in "$WORLDS" "$CALIBRATION" "$IMPORT"; do
  if [[ ! -f "$required" ]]; then
    ...
    echo "Complete/adopt the broad campaign first." >&2
    exit 2
  fi
done
...
echo "============================================================"
echo "MAVEB reviewer-v3 certificate-budgeted practical evidence"
echo "============================================================"
echo "Git SHA                : $HEAD_SHA"
echo "Prepared-world root    : $SOURCE_ROOT"
echo "Output                 : $OUT"
echo "Scenes / dataset       : $SCENES_PER_DATASET"
echo "Residual budget fraction: $RESIDUAL_BUDGET_FRACTION of remaining slack"
echo "Reuse                  : $REUSE"
...
echo
echo "Protocol:"
echo "  - same deterministic real-world selection and physical edits as reviewer-v2"
echo "  - same epsilon ladder: 0.25,0.5,1,2,4,8,16,32 / 255"
echo "  - no fixed omitted-Gaussian percentage"
echo "  - residual budget = fixed fraction × max(0, epsilon - production resolved bound)"
echo "  - oracle chooses the largest deterministic nested omitted subset fitting that budget"
echo "  - v2 evidence remains immutable and separate"
echo
...
if ! "$PYTHON" "$STORAGE_DOCTOR" --repo "$ROOT" --minimum-free-gib "$MIN_FREE_GIB"; then
  echo "Insufficient free disk for reviewer-v3 execution." >&2
  echo "Run: \"$PYTHON\" \"$STORAGE_DOCTOR\" --repo \"$ROOT\" --cleanup-safe --minimum-free-gib 0" >&2
  exit 3
fi

FREEZE_KEY="$("$PYTHON" "$CACHE_KEY"   --label reviewer-stress-freeze-v3-budgeted-residual   --file "$WORLDS"   --file "$CALIBRATION"   --file "$ROOT/benchmarks/scripts/cbrc_freeze_reviewer_budgeted_v3.py"   --file "$ROOT/benchmarks/scripts/cbrc_freeze_reviewer_stress.py"   --file "$ROOT/benchmarks/scripts/cbrc_prepare_campaign_v2.py"   --file "$ROOT/benchmarks/scripts/cbrc_prepare_real_campaign.py"   --value "scenes_per_dataset=$SCENES_PER_DATASET"   --value "residual_budget_fraction=$RESIDUAL_BUDGET_FRACTION")"

step 2 "Freeze reviewer-v3 certificate-budgeted matrix"
if cache_hit freeze "$FREEZE_KEY"    && [[ -f "$FREEZE/reviewer-stress-v3-campaign.json" ]]    && [[ -f "$FREEZE/REVIEWER_STRESS_V3_FREEZE.json" ]]; then
  echo "  [██████████████████████████████] 100.00% | CACHE | reviewer-v3 matrix reused"
else
  rm -rf "$FREEZE"
  mkdir -p "$FREEZE"
  "$PYTHON" benchmarks/scripts/cbrc_freeze_reviewer_budgeted_v3.py     --worlds "$WORLDS"     --output-dir "$FREEZE"     --scenes-per-dataset "$SCENES_PER_DATASET"     --work-cost-model "$CALIBRATION"     --residual-budget-fraction "$RESIDUAL_BUDGET_FRACTION"
  cache_done freeze "$FREEZE_KEY"
fi

CASE_COUNT="$("$PYTHON" - "$FREEZE/reviewer-stress-v3-campaign.json" <<'PY'
import json,sys
from pathlib import Path
...
step 3 "Execute oracle-checked reviewer-v3 cases"
echo "  This step is resumable case-by-case."
export MAVEB_ORACLE_CACHE_DIR="${MAVEB_ORACLE_CACHE_DIR:-$CAMPAIGN/.oracle-cache}"
"$PYTHON" benchmarks/scripts/cbrc_campaign.py   --campaign "$FREEZE/reviewer-stress-v3-campaign.json"   --freeze-provenance "$FREEZE/REVIEWER_STRESS_V3_FREEZE.json"   --oracle "$ORACLE"   --revision-tool "$REVISION"   --git-sha "$HEAD_SHA"   --output-dir "$CAMPAIGN"   --resume   --invalidate-stale-resume   --workers "$CASE_WORKERS"

AUDIT_KEY="$("$PYTHON" "$CACHE_KEY"   --label reviewer-evidence-audit-v3-budgeted-residual   --file "$FREEZE/reviewer-stress-v3-campaign.json"   --file "$CAMPAIGN/campaign-rows.jsonl"   --file "$ROOT/research/analysis/cbrc_reviewer_evidence.py")"

step 4 "Analyze certified non-zero residuals and tolerance behavior"
if cache_hit audit "$AUDIT_KEY"    && [[ -f "$ANALYSIS/REVIEWER_EVIDENCE_AUDIT.json" ]]; then
  echo "  [██████████████████████████████] 100.00% | CACHE | reviewer-v3 audit reused"
else
  rm -rf "$ANALYSIS"
  mkdir -p "$ANALYSIS"
  "$PYTHON" research/analysis/cbrc_reviewer_evidence.py     --campaign "$FREEZE/reviewer-stress-v3-campaign.json"     --rows "$CAMPAIGN/campaign-rows.jsonl"     --output-dir "$ANALYSIS"
  cache_done audit "$AUDIT_KEY"
fi

ROWS_KEY="$("$PYTHON" "$CACHE_KEY" --label reviewer-rows-v3-budgeted --file "$CAMPAIGN/campaign-rows.jsonl")"
VISUAL_KEY="$("$PYTHON" "$CACHE_KEY"   --label reviewer-visual-package-v3-budgeted   --file "$IMPORT"   --file "$ANALYSIS/REVIEWER_EVIDENCE_AUDIT.json"   --file "$ROOT/research/analysis/cbrc_reviewer_visuals.py"   --value "campaign_rows_sha=$ROWS_KEY")"
...
then
  ...
  echo "Install once with: \"$PYTHON\" -m pip install Pillow" >&2
  exit 4
fi

step 5 "Build reviewer-v3 captured-scene figures"
if cache_hit visuals "$VISUAL_KEY"    && [[ -f "$VISUALS/REVIEWER_VISUALS.json" ]]; then
  echo "  [██████████████████████████████] 100.00% | CACHE | reviewer-v3 visuals reused"
else
  rm -rf "$VISUALS"
  mkdir -p "$VISUALS"
...
print()
print("============================================================")
print("REVIEWER-V3 EVIDENCE SUMMARY")
print("============================================================")
for key in (
    "recordCount",
    "localCases",
...
if not audit["reviewerEvidenceReady"]:
    print(
        "Reviewer-v3 remains scientifically valid, but one or more practical "
        "evidence targets remain OPEN. Do not delete or retune frozen cases."
    )
PY
...
Artifacts:
  Campaign     : $FREEZE/reviewer-stress-v3-campaign.json
...
Force a fresh v3 run:
  MAVEB_REVIEWER_V3_REUSE=0 bash run_reviewer_stress_v3.sh

Scientific rule:
  v2 remains frozen negative evidence. v3 uses a separately frozen algorithmic
  repair-budget rule and must not be post-hoc retuned after outcomes are seen.
EOF
