...
if cache_hit freeze "$FREEZE_KEY"    && [[ -f "$FREEZE/reviewer-stress-campaign.json" ]]    && [[ -f "$FREEZE/REVIEWER_STRESS_FREEZE.json" ]]; then
  ...
  rm -rf "$FREEZE"
  mkdir -p "$FREEZE"
  "$PYTHON" benchmarks/scripts/cbrc_freeze_reviewer_stress_v3.py     --worlds "$WORLDS"     --output-dir "$FREEZE"     --scenes-per-dataset "$SCENES_PER_DATASET"     --work-cost-model "$CALIBRATION"
