...
echo "==> [1/10] Fetching/verifying four-scene public RGB/COLMAP set"
...
echo "==> [2/10] Seeding four persistent MAVEB worlds from ordinary-RGB SfM"
...
echo "==> [3/10] Building production/oracle/calibration/sparse-index tools"
cmake --preset ci
...
echo "==> [4/10] Freezing hardware work calibration"
...
echo "==> [5/10] Benchmarking sparse candidate discovery"
...
echo "==> [6/10] Freezing the 60-case campaign before reading outcomes"
rm -rf "$FROZEN"
mkdir -p "$FROZEN"
...
echo "==> [7/10] Running CBRC + FULL/EXACT/heuristics + ablations + parity"
rm -rf "$RESULTS"
mkdir -p "$RESULTS"
set +e
