...
echo "==> [C] Mechanism-isolation ablation stress suite"
FINAL="$ROOT/build/paper-grade-final"
mkdir -p "$FINAL"
"$PYTHON" research/experiments/cbrc_ablation_stress_suite.py \
  --output "$FINAL/ABLATION_STRESS_STATUS.json"

echo "==> [D] Cross-representation coverage figure"
...
Ablation stress  : build/paper-grade-final/ABLATION_STRESS_STATUS.json
