...
A zero selected residual could be misleading if the edits were visually insignificant to begin with, so we checked that separately. Before repair, 56 of the 60 source edits were already above the protected RGB tolerance. This does not make the campaign difficult in every possible sense, but it does rule out the simplest explanation for the zero post-repair residual: that almost all edits were below the quantity-of-interest threshold anyway.
...
Table~\ref{tab:ablations} shows a less tidy result than one might hope for, and we report it as such. Five structural or analytic ablations are neutral on these 60 real cases. Replacing the exterior treatment with a global-norm tail remains certificate-safe, but it sends every case to \FULL{}. Removing fallback fails in the other direction: it never chooses \FULL{}, and only 26.7\% of the cases pass the certificate. The separate mechanism-isolation suite contains seven deterministic synthetic stress cases, one for each v1 mechanism, and all seven move in the expected safety/work direction. Those cases show that the mechanisms are falsifiable; they do not tell us the real-world effect size of each mechanism.
...
The certificate is only as trustworthy as the assumptions behind its edges. Any exact dependency needed to reproduce repaired state has to appear in \hard{} closure, and each \analytic{} edge has to remain a valid finite-change upper bound over the declared revision domain. Changes to the representation, shader, temporal rule, or ownership scheme can invalidate earlier calibration. In practice, that means bound provenance has to be versioned and regression-tested rather than treated as permanent metadata.
...
The broader point is not that persistent captured worlds should always update locally. It is that locality can be made into a runtime decision instead of a fixed policy. A system does not have to choose only between rebuilding everything and trusting a heuristic. If its dependencies can be separated into exact relations and relations with conservative bounds, the decision to stay local can carry a falsifiable output contract. The same idea is not limited to Gaussian rendering; caches, temporal histories, spatial acceleration structures, compressed representations, and publication buffers can all participate when their dependency semantics are made explicit.
...
\section{Conclusion}
We introduced Criticality-Bounded Revision Cones as a fail-closed way to decide when selective recomputation is defensible in a persistent captured world. Here, criticality is the worst normalized certificate-to-tolerance ratio over the protected outputs. \method{} keeps exact dependencies separate from bounded finite-change influence, ties the unrepaired exterior to explicit output tolerances, checks regional work against a full baseline, and independently replays the selected result against a full-after oracle. The frozen public and trained-representation experiments show that local repair and full fallback can live under the same auditable contract. They also reveal what is still missing: candidate discovery is close to global in the frozen path, and we do not yet have a paired wall-clock speedup result. The idea we want to carry forward is therefore fairly simple. Repair locally when the system can justify what it leaves alone; rebuild when it cannot.
