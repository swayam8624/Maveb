...
A \texttt{LocalityLedger} is the counter source of truth across observation, TSDF, mesh, texture-page, material, Gaussian, GPU-publication, and temporal domains. Ledger/mesher disagreement is an error. Native counts stay in their own units until a versioned calibration converts the domains used by an experiment into a scalar work model. The frozen public campaign calibrates four measured domains---Gaussian inspection, Gaussian update, GPU publication bytes, and temporal invalidation pixels---rather than all ledger domains. This prevents a local-looking metric in one measured subsystem from hiding a full-world stage among the measured campaign domains.

\paragraph{Independent oracle.}
The Gaussian oracle is deliberately independent of the planner that selected the revision. It renders the before and equivalent full-after states, then forms a support-replay output by taking full-after values on the selected repair support and retaining before-state values outside that support. The resulting per-pixel exterior residual is reported beside the emitted bound, and the strict evaluator enforces Eq.~\ref{eq:contract}. If the production and offline certificates disagree, or if the measured exterior residual exceeds the emitted certificate, we count that as a correctness failure. Evidence bundles are hashed, and the canonical campaign records workflow IDs, artifact hashes, source hashes, and native/reference planner parity.
...
A zero selected residual would be less informative if the edits were visually negligible to begin with, so we checked that separately. Before repair, 56 of the 60 source edits were already above the protected RGB tolerance. This rules out the simplest explanation that almost all edits were below the quantity-of-interest threshold anyway.
