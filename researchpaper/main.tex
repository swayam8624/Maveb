...
The evaluation asks seven questions: certificate validity against full-after replay (RQ1), \LOCAL{}/\FULL{} crossover (RQ2), calibrated work (RQ3), locality of the campaign-measured domains (RQ4), baseline behavior (RQ5), transfer to trained 3DGS (RQ6), and whether exact sparse discovery removes the full-scan bottleneck (RQ7).
...
For each selected repair, the campaign stores the source edit, chosen cone, decision, certificate, support-replay full-reference residual, native and calibrated work, and provenance. Fallbacks stay in the dataset and in the summary statistics; they are part of the method's behavior, not failed trials to be filtered out.
...
A zero support-replay residual would be less informative if the edits were visually negligible to begin with, so we checked that separately. Before repair, 56 of the 60 source edits were already above the protected RGB tolerance. This rules out the simplest explanation that almost all edits were below the quantity-of-interest threshold anyway.
