#!/usr/bin/env python3
"""Freeze reviewer-targeted CBRC graded-residual campaign v3.
...
EPSILON_LEVELS_255 = v2.EPSILON_LEVELS_255
RESIDUAL_SCALES = (0.0, 1.0 / 1024.0, 1.0 / 256.0, 1.0 / 64.0)
...
def build(
    ...
    *,
            ...
            for family_index, family in enumerate(EDIT_FAMILIES):
                ...
                for residual_scale in residual_scales:
                    ...
                    for epsilon_255 in epsilon_levels_255:
                        ...
                        case_id = (
                            f"reviewerv3--{dataset_id}--{family}--{profile['name']}--"
    ...
    campaign = {
        "schemaVersion": 1,
        "campaignId": "maveb-cbrc-reviewer-graded-residual-v3",
        "protocol": "post-reviewer-graded-residual-v3",
        ...
        "cases": cases,
        "freeze_note": (
            "Reviewer graded-residual v3 protocol frozen before v3 outcomes are inspected. "
            "v2 remains immutable negative evidence for whole-Gaussian omission. v3 keeps "
            "the same physical edit families and severity profiles, adds an exact-repair "
            "control, and uses a fixed continuous residual-amplitude ladder "
            "(0, 1/1024, 1/256, 1/64). Each stress key holds scene, entity, edit, severity, "
            "and residual scale fixed while only epsilon changes. No v3 case may be removed "
            "or retuned after LOCAL/FULL decisions or residuals are observed."
        ),
    }
    provenance = {
        "schemaVersion": 1,
        "artifact": "maveb-cbrc-reviewer-graded-residual-v3-freeze",
        "generator": "benchmarks/scripts/cbrc_freeze_reviewer_stress_v3.py",
        ...
        "selectionRule": "same deterministic scene selection as reviewer-v2",
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Freeze reviewer-targeted graded residual/crossover v3 cases"
    )
