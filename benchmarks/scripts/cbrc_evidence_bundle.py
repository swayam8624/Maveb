...
def bundle(
    *,
    ...
    repair_residual_scale: float = 0.0,
) -> dict:
    if epsilon < 0:
        raise ValueError("epsilon must be non-negative")
    if not 0.0 <= repair_omit_fraction < 1.0:
        raise ValueError("repair_omit_fraction must be in [0,1)")
    if not 0.0 <= repair_residual_scale <= 1.0:
        raise ValueError("repair_residual_scale must be in [0,1]")
    ...
    elif repair_residual_scale > 0.0:
        manifest_payload["repair_residual_scale"] = float(repair_residual_scale)
        manifest_payload["repair_mode"] = "certified-graded-residual-v1"
...
def main() -> int:
    ...
    parser.add_argument("--repair-residual-scale", type=float, default=0.0)
