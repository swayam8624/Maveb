#!/usr/bin/env python3
"""Freeze reviewer-targeted CBRC opacity-delta certificate campaign v5.
...
v5 is a separate, pre-specified certificate validation protocol frozen before
v5 outcomes are inspected. It tests the new opacity-delta residual certificate
against a translation control while preserving the v4 locality profiles. The
epsilon and residual-amplitude ladders are denser so bound scaling and any
decision crossover can be measured rather than inferred. v3/v4 remain immutable.
"""
...
def build(
    ...
    *,
    ...
    if not residual_scales or residual_scales[0] != 0.0:
        raise ValueError("v5 residual ladder must start with exact repair scale 0")
    if any(scale < 0.0 or scale > 1.0 for scale in residual_scales):
        raise ValueError("v5 residual scales must lie in [0,1]")
    if len(set(residual_scales)) != len(residual_scales):
        raise ValueError("v5 residual scales must be unique")
            ...
            for family_index, family in enumerate(EDIT_FAMILIES):
                ...
                for residual_scale in residual_scales:
                    ...
                    for epsilon_255 in epsilon_levels_255:
                        ...
                        case_id = (
                            f"reviewerv5--{dataset_id}--{family}--{profile['name']}--"
    ...
    campaign = {
        "schemaVersion": 1,
        "campaignId": "maveb-cbrc-reviewer-opacity-delta-v5",
        "protocol": "post-reviewer-opacity-delta-v5",
        ...
        "cases": cases,
        "freeze_note": (
            "Reviewer opacity-delta certificate v5 is frozen before v5 outcomes are inspected. "
            "It compares opacity edits, which are eligible for the new delta-sensitive "
            "certificate, against translation controls that remain on the conservative "
            "opacity-envelope certificate. Locality profiles and edit magnitudes remain "
            "controlled. Epsilon is {1,2,4,8,16,32}/255 and residual scale is "
            "{0,1/4096,1/1024,1/256}. v3/v4 remain immutable; no v5 case may be deleted "
            "or retuned after outcomes are observed."
        ),
    }
    provenance = {
        "schemaVersion": 1,
        "artifact": "maveb-cbrc-reviewer-opacity-delta-v5-freeze",
        "generator": "benchmarks/scripts/cbrc_freeze_reviewer_certificate_v5.py",
        ...
        "selectionRule": "same deterministic scene selection as reviewer-v2/v3/v4; certificate protocol frozen before v5 outcomes",
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Freeze reviewer-targeted opacity-delta certificate/crossover v5 cases"
    )
