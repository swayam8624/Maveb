...
Dataset roots are discovered in this order: an explicit dataset-specific `MAVEB_*` environment variable, then canonical directories under `MAVEB_DATA` (for example `ARKitScenes`, `3RScan`, `BonnRGBD`, or `graphdeco-pretrained/models`). If raw sources are unavailable, fast mode also checks prior `broad-benchmark-paper`, `broad-benchmark-smoke`, and `broad-benchmark` prepared-world manifests and can reuse still-valid `.aetherworld` files without reconstructing them. This fallback is development-only; the full publication campaign never silently substitutes an older prepared-world set.
